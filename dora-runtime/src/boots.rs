...
use dora_bytecode::{
    BytecodeType, BytecodeTypeArray, ClassId, FunctionId, FunctionKind, GlobalId, StructId, TraitId,
};
...
use crate::vm::{
    create_class_instance, create_struct_instance, get_concrete_tuple_bty, get_vm, impls,
    CodeDescriptor, VM,
};
...
pub const BOOTS_NATIVE_FUNCTIONS: &[(&'static str, *const u8)] = &[
    ...
    (
        "boots::interface::getStructFieldOffsetRaw",
        get_struct_field_offset_raw as *const u8,
    ),
    (
...
extern "C" fn get_struct_field_offset_raw(data: Handle<UInt8Array>) -> i32 {
    let vm = get_vm();
    ...
    let mut reader = ByteReader::new(serialized_data);
    let struct_id = StructId(reader.read_u32());
    let type_params = decode_bytecode_type_array(&mut reader);
    let field_id = reader.read_u32();
    assert!(!reader.has_more());

    let struct_instance_id = create_struct_instance(vm, struct_id, type_params);
    let struct_instance = vm.struct_instances.idx(struct_instance_id);
    struct_instance.fields[field_id as usize].offset
}

extern "C" fn get_tuple_element_offset_raw(data: Handle<UInt8Array>) -> i32 {
    let vm = get_vm();
    ...
    let mut reader = ByteReader::new(serialized_data);
    let tuple_ty = decode_bytecode_type(&mut reader);
    let subtype_idx = reader.read_u32();
    assert!(!reader.has_more());
