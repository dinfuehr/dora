...
pub struct NativeMethods {
    ...
    fatal_error_trampoline: Option<Address>,
...
impl NativeMethods {
    ...
    pub fn fatal_error_trampoline(&self) -> Address {
        self.fatal_error_trampoline.expect("uninitialized field")
    }
...
pub fn setup_builtin_natives(vm: &mut VM) {
    ...
    let ifct = NativeFct {
        target: NativeTarget::Address(Address::from_ptr(stdlib::trap as *const u8)),
    ...
    let ifct = NativeFct {
        target: NativeTarget::Address(Address::from_ptr(stdlib::unreachable as *const u8)),
