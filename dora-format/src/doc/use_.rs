...
use crate::doc::utils::{
    collect_node, eat_token, has_comment, is_node, is_token, natural_cmp, print_node, print_token,
    print_token_opt,
};
...
fn use_tree_sort_key(path: &AstUseTree) -> (u8, String) {
    match path.target() {
        ...
        Some(AstUseTarget::UseAs(use_as)) => (1, use_as.original_name().text().to_string()),
        None => (1, String::new()),
    }
}
...
mod tests {
    ...
    fn formats_use_nested_group() {
        ...
        let expected = "use super::foo::bar::baz::{c::{d, e, f}, a as x, b};\n";
        assert_source(input, expected);
    }

    #[test]
    fn sorts_use_aliases_by_original_name() {
        let input = "use self::{b as a, a as z, item as z, item as a};";
        let expected = "use self::{a as z, b as a, item as z, item as a};\n";
        assert_source(input, expected);
    }
