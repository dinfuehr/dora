...
use crate::doc::utils::{
    collect_node, eat_token_opt, is_node, is_token, print_node, print_token, print_token_opt,
};
use crate::with_iter;
...
pub(crate) fn format_use_group(node: AstUseGroup, f: &mut Formatter) {
    with_iter!(node, f, |iter, opt| {
        ...
        while !is_token(f, &mut iter, R_BRACE) {
            elements.push(collect_node::<AstUsePath>(f, &mut iter));
            eat_token_opt(f, &mut iter, COMMA);
        }
...
pub(crate) fn format_use_path(node: AstUsePath, f: &mut Formatter) {
    with_iter!(node, f, |iter, opt| {
        while is_token(f, &mut iter, IDENTIFIER) {
            print_token(f, &mut iter, IDENTIFIER, &opt);
            print_token_opt(f, &mut iter, COLON_COLON, &opt);
        }
        print_node::<AstUseTarget>(f, &mut iter);
    });
}
