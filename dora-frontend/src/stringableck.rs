...
pub fn create_impls(sa: &mut Sema) {
    for target in targets(sa) {
        create_stringable_impl(sa, target);
    }
}

pub fn check_fields(sa: &Sema) {
    for target in targets(sa) {
        check_stringable_fields(sa, target);
    }
}
...
fn targets(sa: &Sema) -> Vec<DerivedTarget> {
    ...
    targets.extend(
        sa.classes
            .iter()
            .filter(|(_, class)| class.derive_stringable)
            .map(|(id, _)| DerivedTarget::Class(id)),
    );
    targets.extend(
        sa.structs
            .iter()
            .filter(|(_, struct_)| struct_.derive_stringable)
            .map(|(id, _)| DerivedTarget::Struct(id)),
    );
    targets.extend(
        sa.enums
            .iter()
            .filter(|(_, enum_)| enum_.derive_stringable)
...
fn create_stringable_impl(sa: &mut Sema, target: DerivedTarget) {
    let (
        package_id,
        module_id,
        file_id,
        ...
        (
            ...
            element.module_id(),
            element.file_id(),
            target.annotation_span(sa, "Stringable"),
            element.span(),
    ...
    let stringable_trait_id = sa.known.traits.stringable();
    let trait_ty = TraitType::from_trait_id(stringable_trait_id);
    ...
    let method_name = sa.interner.intern("to_string");
    ...
    let return_type = SourceType::Class(sa.known.classes.string(), SourceTypeArray::empty());
    ...
    method.derived_method = Some(DerivedMethod::Stringable(target));
...
fn check_stringable_fields(sa: &Sema, target: DerivedTarget) {
    let element = target.element(sa);
    ...
    for field_id in target.field_ids(sa) {
        ...
        if !field_ty.is_error()
            && !implements_trait(sa, field_ty.clone(), element, stringable_trait_ty.clone())
        {
            sa.report(
                element.file_id(),
                field.span(),
