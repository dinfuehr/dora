...
use crate::{
    SourceType, SourceTypeArray, TraitType, TypeArgs, find_super_trait_ty, replace_type,
...
fn check_call_arguments_with_expected_inner(
    ...
    // Second pass: check compatibility. Copy the call argument data since inference mutates `ck`.
    let call_args = ck
        .call_args(call_expr_id)
        .iter()
        .map(|arg| (arg.expr, arg.name))
        .collect::<Vec<_>>();

    // Check for unexpected named arguments
    for (idx, (_, name)) in call_args.iter().enumerate() {
        if name.is_some() {
        ...
        .enumerate()
    {
        ...
        let param_ty = if infer_type_params {
            ck.resolve_type(param_ty.clone())
        } else {
            param_ty.clone()
        };
    ...
    if call_args.len() < no_regular_params {
        ...
        for idx in no_regular_params..call_args.len() {
            ...
            let variadic_ty = if infer_type_params {
                ck.resolve_type(variadic_ty.clone())
            } else {
                variadic_ty.clone()
            };
...
fn check_expr_call_qualified_path(
    ...
    let fct_type_param_refs = method_segment.type_params.clone();
...
fn check_expr_call_index(
    ...
    if let Some(impl_match) = impl_match {
        ...
        let return_type = specialize_type(ck.sa, method.return_type(), &type_params);
        ck.body.set_ty(expr_id, return_type.clone());
...
fn check_expr_call_fct(
    ...
    let (mut type_params, type_variables) = if type_params.is_empty() {
        assert!(type_variables.is_empty());
        ck.create_implicit_type_variables(type_param_definition.own_type_params_len(), expr_id)
    } else {
        (type_params, type_variables)
    };
    type_params = prepare_call_type_params(ck, expr_id, fct, None, type_params);
    ...
    let ty = specialize_ty_for_call(ck.sa, fct.return_type(), ck.element, &type_args);
    ck.body.set_ty(expr_id, ty.clone());

    let call_type = CallType::Fct(fct_id, type_args);
    ck.body.insert_call_type(expr_id, Rc::new(call_type));
...
fn check_expr_call_struct(
    ...
    let type_params =
        prepare_call_type_params(ck, expr_id, struct_, expected_type_params, type_params);
...
fn check_expr_call_class(
    ...
    let type_param_definition = cls.type_param_definition(ck.sa);
    let (type_params, type_variables) = if type_params.is_empty() {
        assert!(type_variables.is_empty());
        ck.create_implicit_type_variables(type_param_definition.own_type_params_len(), expr_id)
    } else {
    ...
    let expected_type_params = match &expected_ty {
        SourceType::Class(expected_id, expected_type_params) if *expected_id == cls_id => {
            Some(expected_type_params)
        }
        _ => None,
    };
    let type_params = prepare_call_type_params(ck, expr_id, cls, expected_type_params, type_params);
...
fn check_expr_call_enum_variant(
    ...
    let type_param_definition = enum_.type_param_definition(ck.sa);
    let (type_params, type_variables) = if type_params.is_empty() {
        assert!(type_variables.is_empty());
        ck.create_implicit_type_variables(type_param_definition.own_type_params_len(), expr_id)
    } else {
    ...
    let expected_type_params = match &expected_ty {
        SourceType::Enum(expected_id, expected_type_params) if *expected_id == enum_id => {
            Some(expected_type_params)
        }
        _ => None,
    };
    let type_params =
        prepare_call_type_params(ck, expr_id, enum_, expected_type_params, type_params);
...
fn check_expr_call_path(
    ...
    let is_enum_variant = match &resolution {
        PathResolution::Symbol(SymbolKind::Enum(enum_id)) => ck
            .sa
            .enum_(*enum_id)
            .name_to_value()
            .contains_key(&interned_method_name),
        _ => false,
    };
