...
use crate::sema::{ExprId, IdentType};

pub(super) fn gen_expr_self(g: &mut AstBytecodeGen, expr_id: ExprId, dest: DataDest) -> Register {
    if g.is_lambda {
        ...
        gen_expr_path_context(g, level, context_idx, dest, g.loc_for_expr(expr_id))
    } else {
