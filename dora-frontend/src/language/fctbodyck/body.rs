...
impl<'a> TypeCheck<'a> {
    ...
    fn check_stmt_let(&mut self, s: &ast::StmtLetType) {
        ...
        if s.expr.is_some() {
                ...
                && !defined_type.allows(self.sa, expr_type.clone())
            {
                ...
                self.sa.diag.lock().report(self.file_id, s.span, msg);
            }
    ...
    fn check_stmt_for(&mut self, stmt: &ast::StmtForType) {
        ...
        self.sa
            .diag
            .lock()
            .report(self.file_id, stmt.expr.span(), msg);
    ...
    fn check_stmt_while(&mut self, stmt: &ast::StmtWhileType) {
        ...
        if !expr_type.is_error() && !expr_type.is_bool() {
            ...
            self.sa.diag.lock().report(self.file_id, stmt.span, msg);
        }
    ...
    fn check_fct_return_type(&mut self, span: Span, expr_type: SourceType) {
        ...
        if !expr_type.is_error() && !fct_type.allows(self.sa, expr_type.clone()) {
            ...
            self.sa.diag.lock().report(self.file_id, span, msg);
        }
    }
    ...
    fn check_expr_match(
        ...
        for case in &node.cases {
            ...
            if result_type.is_error() {
                ...
                self.sa
                    .diag
                    .lock()
                    .report(self.file_id, case.value.span(), msg);
            }
        ...
        if used_variants.count_ones(..) != 0 {
            ...
            self.sa.diag.lock().report(self.file_id, node.span, msg);
        }
                    ...
                    Ok(Sym::EnumVariant(enum_id, variant_idx)) => {
                        if Some(enum_id) == expr_enum_id {
                            ...
                            self.sa
                                .diag
                                .lock()
                                .report(self.file_id, ident.path.span, msg);
                        }
                    }
        ...
        match sym {
            ...
            _ => {
                self.sa
                    .diag
                    .lock()
                    .report(self.fct.file_id, e.span, ErrorMessage::ValueExpected);
                SourceType::Error
            }
        }
    }
    ...
    fn check_expr_assign_field(&mut self, e: &ast::ExprBinType) {
        ...
        self.sa
            .diag
            .lock()
            .report(self.file_id, field_expr.op_span, msg);
        ...
        if let Some(expr_ident) = callee.to_ident() {
            ...
            if !type_params.is_empty() {
                ...
                self.sa
                    .diag
                    .lock()
                    .report(self.file_id, e.callee.span(), msg);
            }
    ...
    fn check_expr_call_path(
        ...
        let method_name = if let Some(method_name_expr) = method_expr.to_ident() {
            ...
            self.sa
                .diag
                .lock()
                .report(self.file_id, method_expr.span(), msg);
        ...
        if let Some(expr_path) = expr.to_path() {
            ...
            let element_name = if let Some(ident) = expr_path.rhs.to_ident() {
                ...
                self.sa
                    .diag
                    .lock()
                    .report(self.file_id, expr_path.rhs.span(), msg);
                return Err(());
            };
    ...
    fn check_enum_value_without_args(
        ...
        if !enum_accessible_from(self.sa, enum_id, self.module_id) {
            let msg = ErrorMessage::NotAccessible(enum_.name(self.sa));
            self.sa.diag.lock().report(self.file_id, expr_span, msg);
        }
        ...
        if let Some(ident) = e.callee.to_ident() {
            ...
            let container_name = if let Some(container_expr) = path.lhs.to_ident() {
                ...
                self.sa
                    .diag
                    .lock()
                    .report(self.file_id, path.lhs.span(), msg);
            ...
            let method_name = if let Some(ident) = path.rhs.to_ident() {
                ...
                self.sa
                    .diag
                    .lock()
                    .report(self.file_id, path.rhs.span(), msg);
    ...
    fn check_stmt_break_and_continue(&mut self, stmt: &ast::StmtData) {
        if !self.in_loop {
            self.sa
                .diag
                .lock()
                .report(self.fct.file_id, stmt.span(), ErrorMessage::OutsideLoop);
        }
    }
}
