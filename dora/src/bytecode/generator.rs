...
impl<'a> AstBytecodeGen<'a> {
    fn generate(mut self, ast: &Function) -> BytecodeFunction {
        ...
        if self.fct.has_self() {
            ...
            if !var_ty.is_unit() {
                ...
                let bty = BytecodeType::from_ty(self.vm, var_ty);
        ...
        for param in &ast.params {
            ...
            if ty.is_unit() {
                ...
                let bty = BytecodeType::from_ty(self.vm, ty);
                let reg = self.alloc_var(bty);
                self.var_registers.insert(var_id, reg);
                arguments += 1;
            }
    ...
    fn visit_stmt_for_pattern_setup(&mut self, pattern: &LetPattern) {
        match pattern {
            LetPattern::Ident(ref ident) => {
                ...
                if !var_ty.is_unit() {
                    let bty: BytecodeType = BytecodeType::from_ty(self.vm, var_ty);
    ...
    fn visit_expr_if(&mut self, expr: &ExprIfType, dest: DataDest) -> Register {
        ...
        if let Some(ref else_block) = expr.else_block {
            let dest = if ty.is_unit() {
                ...
                self.ensure_register(dest, BytecodeType::from_ty(self.vm, ty))
            };
    ...
    fn visit_expr_dot(&mut self, expr: &ExprDotType, dest: DataDest) -> Register {
        ...
        let field_bc_ty: BytecodeType = BytecodeType::from_ty(self.vm, field_ty);
    ...
    fn emit_intrinsic_bin(
        ...
        match intrinsic {
            ...
            Intrinsic::Int32AddUnchecked => {
                self.gen.emit_add_int32_unchecked(dest, lhs_reg, rhs_reg)
            }
            ...
            Intrinsic::Int32DivUnchecked => {
                self.gen.emit_div_int32_unchecked(dest, lhs_reg, rhs_reg)
            }
            Intrinsic::Int32Mod => self.gen.emit_mod_int32(dest, lhs_reg, rhs_reg, pos),
            Intrinsic::Int32ModUnchecked => {
                self.gen.emit_mod_int32_unchecked(dest, lhs_reg, rhs_reg)
            }
            ...
            Intrinsic::Int64AddUnchecked => {
                self.gen.emit_add_int64_unchecked(dest, lhs_reg, rhs_reg)
            }
            ...
            Intrinsic::Int64DivUnchecked => {
                self.gen.emit_div_int64_unchecked(dest, lhs_reg, rhs_reg)
            }
            Intrinsic::Int64Mod => self.gen.emit_mod_int64(dest, lhs_reg, rhs_reg, pos),
            Intrinsic::Int64ModUnchecked => {
                self.gen.emit_mod_int64_unchecked(dest, lhs_reg, rhs_reg)
            }
    ...
    fn visit_expr_assign_dot(&mut self, expr: &ExprBinType, dot: &ExprDotType) {
        ...
        let field_ty: Option<BytecodeType> = if field_ty.is_unit() {
            ...
            Some(BytecodeType::from_ty(self.vm, field_ty))
        };
