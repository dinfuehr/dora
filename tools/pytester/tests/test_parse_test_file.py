from pytester.config import BOOTS_CONFIG, CANNON_CONFIG
...
def test_stdout_file_detected(tmp_path, monkeypatch):
    ...
    options = RunnerOptions(force_config=BOOTS_CONFIG)
...
def test_boots_conditional_args_do_not_apply_to_cannon(tmp_path, monkeypatch):
    monkeypatch.setattr("pytester.tests.REPO_ROOT", tmp_path)
    ...
    dora_file.write_text(
        "\n".join(
            [
                '//= runtime-args "--boots-runtime-arg" if boots',
                "fn main() {}",
            ]
        )
        + "\n"
