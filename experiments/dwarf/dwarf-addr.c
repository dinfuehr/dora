    ...
    for (i = 0; i < attrcount; ++i) {
        ...
        */
        ...
        else if (attrcode == DW_AT_high_pc)
        {
          ...
          if (dwarf_whatform(attrs[i], &form, &err))
            die("Error in dwarf_whatform\n");

          switch (form)
          {
          case DW_FORM_addr:
            if (dwarf_formaddr(attrs[i], &highpc, &err))
              die("Error in dwarf_formaddr\n");
            break;

          case DW_FORM_data8:

            if (dwarf_formudata(attrs[i], &offset, &err))
              die("Error in dwarf_formudata\n");
