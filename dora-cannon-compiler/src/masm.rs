...
use dora_compiler::{
    ...
    GcPointTable, Header, InlinedLocation, JumpTable, LocationTable, MachineMode, RelocationEntry,
...
impl MacroAssembler {
    ...
    fn resolve_jump_tables(
        jump_tables: &[Vec<Label>],
        asm: &dora_asm::AssemblerBuffer,
    ) -> Vec<JumpTable> {
        jump_tables
            .iter()
            .map(|targets| JumpTable {
                targets: targets
                    .iter()
