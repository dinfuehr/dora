...
pub struct CannonCodeGen<'a, 'i> {
    ...
    bytecode: Option<&'a BytecodeBody>,
    registers: Vec<BytecodeType>,
    trait_object_thunk: Option<TraitObjectThunkData>,
...
impl<'a, 'i> CannonCodeGen<'a, 'i> {
    pub(super) fn new(
        ...
        CannonCodeGen {
            ...
            bytecode: Some(compilation_data.bytecode_body),
            registers: compilation_data.bytecode_body.registers().to_vec(),
            trait_object_thunk: None,
    ...
    pub(super) fn new_for_trait_object_thunk(
        compilation_data: TraitObjectThunkCompilationData<'a>,
        aot_intrinsics: &'i HashMap<FunctionId, Intrinsic>,
    ) -> CannonCodeGen<'a, 'i> {
        let TraitObjectThunkCompilationData {
            program,
            trait_fct_id,
            trait_object_ty,
            actual_object_ty,
        ...
        assert_eq!(signature.params[0], trait_object_ty);
        assert_eq!(
            signature.type_params[signature.type_params.len() - 1],
            actual_object_ty
        );
        ...
        CannonCodeGen {
            ...
            location: loc,
            emit_debug: options.emit_debug,
            asm: BaselineAssembler::new(program),
            bytecode: None,
            registers,
    ...
    fn bytecode(&self) -> &'a BytecodeBody {
        self.bytecode.expect("missing bytecode")
    }

    pub fn generate(mut self) -> CodeDescriptor {
        self.emit_function_entry();

        let bytecode = self.bytecode();
        read(bytecode.code(), &mut self);
    ...
    pub fn generate_trait_object_thunk(mut self) -> CodeDescriptor {
        self.emit_function_entry();

        let thunk = self
            .trait_object_thunk
            .take()
            .expect("missing trait object thunk");
        ...
        assert!(
            self.intrinsic_for_fct_id(thunk.callee_fct_id).is_none(),
            "trait object thunk target must not be an intrinsic"
        );
        ...
        self.emit_return_generic(thunk.result_reg);
    ...
    fn emit_function_entry(&mut self) {
        if self.emit_debug {
            self.asm.debug();
        }
    ...
    fn emit_load_register(&mut self, src: Register, dest: AnyReg) {
        ...
        self.asm
            .load_mem(self.mode(bytecode_type), dest, Mem::Local(offset));
    }
    ...
    fn emit_store_register(&mut self, src: AnyReg, dest: Register) {
        ...
        self.asm
            .store_mem(self.mode(bytecode_type), Mem::Local(offset), src);
    }
    ...
    fn emit_neg(&mut self, dest: Register, src: Register) {
        ...
        self.asm
            .float_neg(self.mode(bytecode_type), FREG_RESULT, FREG_RESULT);
    ...
    fn emit_checked_neg(&mut self, dest: Register, src: Register) {
        ...
        self.asm
            .int_neg_checked(self.mode(bytecode_type), REG_RESULT, REG_RESULT, loc);
    ...
    fn emit_intrinsic_abs_float(&mut self, dest: Register, src: Register) {
        ...
        self.asm
            .float_abs(self.mode(bytecode_type), FREG_RESULT, FREG_RESULT);
    ...
    fn emit_and(&mut self, dest: Register, lhs: Register, rhs: Register) {
        ...
        self.asm
            .int_and(self.mode(bytecode_type), REG_RESULT, REG_RESULT, REG_TMP1);
    ...
    fn emit_or(&mut self, dest: Register, lhs: Register, rhs: Register) {
        ...
        self.asm
            .int_or(self.mode(bytecode_type), REG_RESULT, REG_RESULT, REG_TMP1);
    ...
    fn emit_xor(&mut self, dest: Register, lhs: Register, rhs: Register) {
        ...
        self.asm
            .int_xor(self.mode(bytecode_type), REG_RESULT, REG_RESULT, REG_TMP1);
    ...
    fn emit_not(&mut self, dest: Register, src: Register) {
        ...
        if bytecode_type == BytecodeType::Bool {
            ...
            self.asm
                .int_not(self.mode(bytecode_type), REG_RESULT, REG_RESULT);
    ...
    fn emit_rol_int(&mut self, dest: Register, lhs: Register, rhs: Register) {
        ...
        let bytecode_type = self.bytecode().register_type(dest);
        self.asm
            .int_rol(self.mode(bytecode_type), REG_RESULT, REG_RESULT, REG_TMP1);
    ...
    fn emit_ror_int(&mut self, dest: Register, lhs: Register, rhs: Register) {
        ...
        let bytecode_type = self.bytecode().register_type(dest);
        self.asm
            .int_ror(self.mode(bytecode_type), REG_RESULT, REG_RESULT, REG_TMP1);
    ...
    fn emit_reinterpret(&mut self, dest: Register, src: Register) {
        ...
        let src_register = result_reg_mode(self.mode(src_type.clone()));
        self.emit_load_register(src, src_register.into());

        let dest_type = self.bytecode().register_type(dest);
        let dest_register = result_reg_mode(self.mode(dest_type.clone()));
    ...
    fn emit_load_enum_element(&mut self, dest: Register, src: Register, idx: ConstPoolIdx) {
        ...
        match self.layout.enum_layout(enum_id, &type_params) {
            ...
            AotEnumLayout::Tagged => {
                let layout = self
                    .layout
                    .enum_variant_layout(enum_id, &type_params, variant_idx);
    ...
    fn emit_load_const(&mut self, dest: Register, const_id: ConstId) {
        ...
        match ty {
            ...
            | BytecodeType::Int64 => {
                ...
                self.asm.load_int_const(self.mode(ty), REG_RESULT, value);
            ...
            BytecodeType::Float32 | BytecodeType::Float64 => {
                ...
                self.asm.load_float_const(self.mode(ty), FREG_RESULT, value);
                self.emit_store_register(FREG_RESULT.into(), dest);
            }
    ...
    fn emit_const_int(&mut self, dest: Register, int_const: i64) {
        ...
        self.asm
            .load_int_const(self.mode(bytecode_type), REG_RESULT, int_const);
    ...
    fn emit_const_float(&mut self, dest: Register, float_const: f64) {
        ...
        self.asm
            .load_float_const(self.mode(bytecode_type), FREG_RESULT, float_const);
    ...
    fn emit_test_generic(&mut self, dest: Register, lhs: Register, rhs: Register, op: CondCode) {
        ...
        if bytecode_type.is_any_float() {
            ...
            self.asm
                .cmp_reg(self.mode(bytecode_type), REG_RESULT, REG_TMP1);
    ...
    fn emit_load_trait_object_receiver(
        &mut self,
        dest: Register,
        object: Register,
        value_ty: BytecodeType,
    ) {
    ...
    fn emit_load_trait_object_receiver_ref(
        &mut self,
        dest: Register,
        object: Register,
        value_ty: BytecodeType,
    ...
    fn emit_new_tuple(&mut self, dest: Register, idx: ConstPoolIdx, arguments: &[Register]) {
        ...
        for field in tuple.fields {
            ...
            let src = self.reg(src);
            let dest = RegOrOffset::Offset(dest_offset + field.offset);
            self.asm.copy_bytecode_ty(field.ty, dest, src);
            arg_idx += 1;
        }
    }
        ...
        match self.layout.enum_layout(enum_id, &type_params) {
            ...
            AotEnumLayout::Tagged => {
                let layout = self
                    .layout
                    .enum_variant_layout(enum_id, &type_params, variant_idx);
                let alloc_size = layout.size as usize;
                ...
                assert_eq!(arguments.len(), layout.fields.len() - 1);
    ...
    fn emit_new_struct(&mut self, dest: Register, idx: ConstPoolIdx, arguments: &[Register]) {
        ...
        assert_eq!(arguments.len(), struct_.fields.len());

        for (field_idx, &arg) in arguments.iter().enumerate() {
            let ty = self.specialize_register_type(arg);
            let field = &struct_.fields[field_idx];
    ...
    fn emit_new_trait_object(&mut self, dest: Register, idx: ConstPoolIdx, src: Register) {
        ...
        let dest = RegOrOffset::RegWithOffset(REG_TMP1, field_offset);
        let src = self.reg(src);
    ...
    fn emit_store_array(&mut self, src: Register, arr: Register, idx: Register) {
        ...
        let element_size = self.size(src_type.clone());
        self.asm
    ...
    fn emit_load_array(&mut self, dest: Register, arr: Register, idx: Register) {
        ...
        match dest_type {
            ...
            BytecodeType::Struct(struct_id, type_params) => {
                let element_size = self.layout.struct_layout(struct_id, &type_params).size;
                self.asm
                    .array_address(REG_TMP1, REG_RESULT, REG_TMP1, element_size);
                let dest_offset = self.register_offset(dest);
    ...
    fn emit_invoke_direct_or_intrinsic(
        ...
        if let Some(intrinsic) = self.intrinsic_for_fct_id(fct_id) {
    ...
    fn emit_invoke_generic(
        ...
        let (ty, trait_fct_id, trait_type_params, pure_fct_type_params) =
            match self.bytecode().const_pool(fct_idx) {
                ConstPoolEntry::Generic {
                    ...
                    let ty = self.specialize_ty(object_type.clone());
                    (
                        ty,
                        *fct_id,
        ...
        let fct = self.program.fct(trait_fct_id);
    ...
    fn emit_intrinsic_option_get_or_panic(
        ...
        match self.layout.enum_layout(enum_id, &type_params) {
            ...
            AotEnumLayout::Tagged => {
                ...
                let layout =
                    self.layout
                        .enum_variant_layout(enum_id, &type_params, some_variant_id);
    ...
    fn size(&self, ty: BytecodeType) -> i32 {
        self.layout.size(ty)
    }
    ...
    fn reg_ty(&self, reg: Register) -> BytecodeType {
        let ty = self.registers[reg.0].clone();
...
impl<'a, 'i> BytecodeVisitor for CannonCodeGen<'a, 'i> {
    ...
    fn visit_get_field_ref(&mut self, dest: Register, obj: Register, field_idx: ConstPoolIdx) {
        ...
        self.emit_get_field_ref(dest, obj, field_idx);
    }

    fn visit_store_ref(&mut self, src: Register, reference: Register) {
        comment!(self, format!("StoreRef {}, {}", src, reference));
    ...
    fn visit_const_string(&mut self, dest: Register, idx: ConstPoolIdx) {
        ...
        self.emit_const_string(dest, idx, value);
    }
    ...
    fn visit_new_tuple(&mut self, dest: Register, idx: ConstPoolIdx, arguments: Vec<Register>) {
        comment!(self, {
            ...
            let tuple_name = display_ty(self.program, &BytecodeType::Tuple(subtypes.clone()));
            format!(
    ...
    fn visit_new_trait_object(&mut self, dest: Register, src: Register, idx: ConstPoolIdx) {
        comment!(self, {
            ...
            let trait_name = display_ty(self.program, trait_ty);
            let object_name = display_ty(self.program, actual_object_ty);
            format!(
